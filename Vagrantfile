...
Vagrant.configure("2") do |config|
  ...
  ############################################################################
  # Install Redis
  config.vm.proviion :shell, path: "vagrant/services/redis/install.sh"
  ...
  config.vm.provision :shell, path: "vagrant/environment/python/install.sh"
  # Configure the shell to automatically load into the virtualenv environment
  config.vm.provision :shell, path: "vagrant/environment/appsec_wtf/configure_default.sh", privileged: false
